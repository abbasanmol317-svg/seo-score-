...
export type ToolId = 
  ...
  | 'meta-tag' | 'serp-preview' | 'og-preview' | 'schema-markup' 
...
export const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    ## 🚀 Optimization Guide
    (Simple, actionable steps to improve speed)`,
  ...
  },
  {
    ...
    ## ⚠️ Risk Analysis
    (Spammy patterns or toxic links)
  ...
  },
  {
    ...
    ## 📊 Comparison Table
    (Metric, Your Site, Competitor, Winner [GOOD/POOR])
  ...
  },
  {
    ...
    ## 📚 Expert Explanation
    (Brief explanation)`,
    placeholder: 'Ask anything about SEO (e.g., "How to improve domain authority?")'
  },
  {
    id: 'seo-dashboard',
    name: 'SEO Dashboard',
...
export async function runTool(toolId: ToolId, input: string) {
  ...
  // For tools that take a URL, we can use urlContext if we want, 
  // but for simplicity and reliability across all tools, we'll just pass the input in the prompt.
  ...
  const response = await ai.models.generateContent({
    ...
    contents: fullPrompt,
    config: {
      tools: [{ urlContext: {} }] // Enable URL context just in case user provides a URL
    }
  });
